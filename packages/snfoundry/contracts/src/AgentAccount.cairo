...
pub trait IAgentAccount<TContractState> {
    ...
    fn is_session_valid(
        self: @TContractState, session_public_key: felt252, block_number: u64,
    ) -> bool;
    ...
    fn check_policy(self: @TContractState, method_selector: felt252, amount: u256) -> bool;
...
mod AgentAccount {
    ...
    use starknet::{get_block_number, get_caller_address, syscalls};
    ...
    impl AgentAccountImpl of IAgentAccount<ContractState> {
        ...
        fn is_session_valid(
            self: @ContractState, session_public_key: felt252, block_number: u64,
        ) -> bool {
            let session = self.session_keys.entry(session_public_key).read();
        ...
        fn set_spending_limit(ref self: ContractState, daily_limit: u256, tx_limit: u256) -> bool {
            self._assert_only_owner();
        ...
        fn check_policy(self: @ContractState, method_selector: felt252, amount: u256) -> bool {
            let policy = self.spending_policy.read();
            let current_block = get_block_number();
        ...
        fn execute_with_session(
            ...
            let verified_key = self
                ._recover_session_key(message_hash, session_signature, session_public_key);
            ...
            assert(
                self.allowed_methods.entry((verified_key, selector)).read(), 'Method not allowed',
            );
        ...
        fn toggle_kill_switch(ref self: ContractState) -> bool {
            ...
            self.emit(KillSwitchToggled { is_active: new_state, toggled_at: get_block_number() });
    ...
    impl InternalImpl of InternalTrait {
        ...
        fn _execute_call(
            ...
            let result = syscalls::call_contract_syscall(to, selector, calldata.span()).unwrap();

            self.emit(TransactionExecuted { to, selector, amount });
        ...
        fn _compute_message_hash(
            self: @ContractState, to: ContractAddress, selector: felt252, amount: u256, nonce: u64,
        ) -> felt252 {
            ...
            let amount_low: felt252 = (amount & 0xffffffffffffffffffffffffffffffff)
                .try_into()
                .unwrap();
            let amount_high: felt252 = (amount / 0x100000000000000000000000000000000)
                .try_into()
                .unwrap();
        ...
        fn _recover_session_key(
            ...
            let is_valid = check_ecdsa_signature(message_hash, provided_session_key, sig_r, sig_s);
