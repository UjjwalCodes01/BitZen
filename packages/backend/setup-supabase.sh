...
echo "╔════════════════════════════════════════╗"
echo "║  BitZen Backend - Supabase Setup      ║"
echo "╚════════════════════════════════════════╝"
echo ""
...
if [ -f .env ]; then
    ...
    echo
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        echo "❌ Setup cancelled. Please update .env manually."
        exit 1
    fi
fi
...
if [[ $DATABASE_URL =~ postgresql://([^:]+):([^@]+)@([^:]+):([^/]+)/(.+) ]]; then
    ...
    echo "✅ Database credentials parsed successfully"
else
    echo "⚠️  Could not parse DATABASE_URL automatically"
...
echo ""
echo "🗄️  Step 4: Checking database tables..."
echo ""
echo "⚠️  IMPORTANT: You need to create tables in Supabase!"
echo ""
echo "Two options:"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    echo "✅ Great! Tables should be ready."
else
    echo "⚠️  Remember to create tables before using the API!"
...
(async () => {
  try {
    ...
    if (tables.rows.length > 0) {
      ...
      console.log('\n⚠️  No tables found. Please run supabase-schema.sql');
    }
